...
public interface StateRepository extends JpaRepository<StateMaster, Integer> {
	...
	@Query("SELECT CASE WHEN COUNT(sm.name) > 0 THEN 'true' ELSE 'false' END FROM StateMaster sm "
			+ " where sm.countryId.id in ?1 and sm.name in ?2")
	...
	@Query("SELECT CASE WHEN COUNT(name) > 0 THEN 'true' ELSE 'false' END FROM StateMaster sm where sm.countryId.id in ?1 and sm.name in ?2 and sm.id <> ?3")	
