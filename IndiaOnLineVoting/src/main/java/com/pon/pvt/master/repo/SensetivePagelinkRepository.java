...
public interface SensetivePagelinkRepository extends JpaRepository<SensetivePagelinkMaster, Integer>{
	...
	//@Query("SELECT sl FROM SensetivePagelinkMaster sl where sl.activateStartDate <= ?1 and sl.activateEndDate >= ?1 and sl.pageUrl=?2")
	
	@Query("SELECT sl FROM SensetivePagelinkMaster sl where sl.activateStartDate <= ?1 and sl.activateEndDate >= ?1 and sl.pageUrl=?2")
	
	SensetivePagelinkMaster getSensetivePagelinkActiveNow(Date now,String pageUrl);
