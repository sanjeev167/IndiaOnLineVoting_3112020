...
public interface CityRepository extends JpaRepository<CityMaster, Integer> {
	...
	@Query("SELECT CASE WHEN COUNT(cm.name) > 0 THEN 'true' ELSE 'false' END FROM CityMaster cm "
			+ " where cm.stateId.id in ?1 and cm.name in ?2")
	...
	@Query("SELECT CASE WHEN COUNT(cm.name) > 0 THEN 'true' ELSE 'false' END FROM CityMaster cm where cm.stateId.id in ?1 and cm.name in ?2 and cm.id <> ?3")	
