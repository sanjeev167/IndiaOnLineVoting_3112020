...
function reloadGrid() {
	...
	window.location.replace("/pvt/master/votersEnrolled/");
...
function searchAndReloadGrid(stateNameId,loksabhaNameId,assemblyNameId,pollingBoothName) {
	clearCheckBoxSelection();
	var table = $('#votersEnrolledId').DataTable();
	table.ajax.url(
			"paginated?stateNameId=" + stateNameId + "&loksabhaNameId=" +loksabhaNameId+ "&assemblyNameId=" +assemblyNameId+ "&pollingBoothName=" +pollingBoothName).load();
...
var baseUrl = '/pvt/master/votersEnrolled';
...
function applyReadOnlyProp() {	
	...
	$('#pollingBoothNameId').attr('disabled', true);
...
function removeReadOnlyProp() {	
	...
	$('#pollingBoothNameId').attr('disabled', false);
...
function showBusinessEerror(fieldErrMsgMap) {	
	...
	$('#pollingBoothNameId_err').html(fieldErrMsgMap.pollingBoothNameId);
...
function saveAndUpdateRecord() { 
	...
	var id = $('#votersEnrolledRecordId').val();	
	var stateNameId = $('#stateNameId').val();	
	var loksabhaNameId = $('#loksabhaNameId').val();
	var assemblyNameId = $('#assemblyNameId').val().trim();
	var pollingBoothNameId = $('#pollingBoothNameId').val().trim();
	...
	//alert("assemblyNameId = "+assemblyNameId);
	//alert("pollingBoothNameId = "+pollingBoothNameId);
	var json = {
			"id" : id,
...
function loadPollingBoothCombo(id,selectedId){	
	...
	.ajax({
		...
		success : function(response) {			 
			if(response.status){				
						//alert("Response is coming ");
				var ele = document.getElementById('pollingBoothNameId');	       
		        for (var i = 0; i < response.comboList.length; i++) {		        	
		            // POPULATE SELECT ELEMENT WITH JSON.
		        	if(selectedId == response.comboList[i]['id'])
		            ele.innerHTML = ele.innerHTML +
