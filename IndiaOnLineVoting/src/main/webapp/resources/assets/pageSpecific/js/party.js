...
function reloadGrid() {
	...
	window.location.replace("/pvt/master/party/");
...
function searchAndReloadGrid(partyTypeId, stateNameId) {
	...
	table.ajax.url(
			"paginated?partyTypeId=" + partyTypeId + "&stateNameId=" +stateNameId+ ""+""+"").load();
	...
	t = $("#partyId").DataTable(
			{
				...
				"aaSorting" : [ [ 3, "asc" ] ],
				"ajax" : {
					"url" : "/pvt/master/party/paginated?partyTypeId=" 
					                                    + partyTypeId + 
...
function loadPartySymbolCombo(){	
	/* stop form from submitting normally */	
	method = 'GET';
	url = "/pvt/master/party/partySymbollist";	
	$
	.ajax({
		type : method,
		url : url,
