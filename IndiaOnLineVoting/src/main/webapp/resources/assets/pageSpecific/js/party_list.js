...
$(document).ready(function() {
	...
	$(document).on("click",".addClass",function() {
		...
		$("#commonModalTitleId").html('Add a party');
	...
	$(document).on("click", ".vClass", function() {			
		...
		applyReadOnlyProp();		
		getIdBasedRecord($(this).attr("href").split('=')[1]);
		$("#commonModalTitleId").html('View a party');
		$('#modal-common').modal('toggle');
	});
	...
	$(document).on("click", "#searchFormButtonId", function(e) {
		var partyTypeId = $('#partyTypeIdId').val();
		var stateNameId = $('#stateNameIdId').val();		
		
		searchAndReloadGrid(partyTypeId, stateNameId);//While searching stateId will be passed not the name
	});
...
function reloadGrid() {
	...
	window.location.replace("/pub/ec/parties/");
...
function searchAndReloadGrid(partyTypeId, stateNameId) {
	clearCheckBoxSelection();
	var table = $('#partyId').DataTable(); 
	
	
	table.ajax.url(
			"/pub/ec/parties/paginated?partyTypeId=" + partyTypeId + "&stateNameId=" +stateNameId+ ""+""+"").load();
...
function GetSelectedRowID() {
	...
	$(".chkIndvRow").each(function() {
		if ($(this).is(':checked')) {
			...
			checkedRowNames.push(selectedRows[0].stateName +" => "+selectedRows[0].partyName + "</br>");
...
var t;
function loadGrid(partyTypeId, stateNameId, stateName,partyName,partyType) {
	...
	t = $("#partyId").DataTable(
			{
				...
				"aaSorting" : [ [ 3, "asc" ] ],
				"ajax" : {
					"url" : "/pub/ec/parties/paginated?partyTypeId=" 
					                                    + partyTypeId + 
				...
				"columns" : [
					...
					
					{
                        "render": function (data, type, JsonResultRow, meta) {
                            return '<img height="50" width="50" src="'+JsonResultRow.base64ImgName+'">';
                        },
                        "searchable" : false,
                    },
...
function cleanAllMsg() {
	...
	$('#partyTypeIdId_err').html("");
	
	$('#stateNameIdId_err').html("");	
	$('#partyNameId_err').html("");
	$('#partySymbolIdId_err').html("");
}
...
function cleanAllHiddenInput() {
	...
	$('#partySymbolIdId').ddslick('select', {index: 0 });//It id is ddslick method
}

//Apply read only on all the fields of the form so that thevalue of the field
//can not be changed.
function applyReadOnlyProp() {
	$('#partyTypeIdId').attr("disabled", true);	
	$('#stateNameIdId').attr('disabled', true);
	$('#partyNameId').prop("readonly", true);
	$('#partySymbolIdId').attr("disabled", true);	
	$('#partySymbolIdId').attr("disabled", true);
...
function removeReadOnlyProp() {
    $('#partyTypeIdId').attr("disabled", false);	
	$('#stateNameIdId').attr('disabled', false);
	$('#partyNameId').prop("readonly", false);
	$('#partySymbolIdId').attr("disabled", false);	
}

//If the form requires anything pre-loaded. it can be done here.
function preparePage() {
   loadCountryCombo();
   loadPartySymbolCombo();
}
...
function loadRecordInForm(response) {
	$('#partyRecordId').val(response.id);	
	//alert(response.partySymbolId);
	//alert(response.partyTypeId);
	$('#partyTypeIdId').val(response.partyTypeId);
	$('#stateNameIdId').val(response.stateNameId);
	$('#partyNameId').val(response.partyName);	
	var imgINdex=response.partySymbolId*1-1;
	$('#partySymbolIdId').ddslick('select', {index: imgINdex });//It id is ddslick method
}
...
function showBusinessEerror(fieldErrMsgMap) {
	$('#partyTypeIdId_err').html(fieldErrMsgMap.partyTypeId);
	
	$('#stateNameIdId_err').html(fieldErrMsgMap.stateNameId);	
	$('#partyNameId_err').html(fieldErrMsgMap.partyName);
	$('#partySymbolIdId_err').html(fieldErrMsgMap.partySymbolId);
}


var partySymbolIdOnImageSelection;
function saveAndUpdateRecord() { 
	...
	var id = $('#partyRecordId').val();
	var partyTypeId = $('#partyTypeIdId').val();
	var partyName = $('#partyNameId').val();
	var stateNameId = $('#stateNameIdId').val();
	
	var partySymbolId = partySymbolIdOnImageSelection;	
	
	
	//alert("partySymbolId = "+partySymbolId);
...
function loadCountryCombo(){	
	...
	.ajax({
		...
		success : function(response) {			
			if(response.status){
				var ele = document.getElementById('stateNameIdId');	       
...
function loadPartySymbolCombo(){	
	/* stop form from submitting normally */	
	method = 'GET';
	url = "/pub/ec/parties/partySymbollist";	
	$
	.ajax({
		type : method,
		url : url,
		...
		success : function(response) {			
			if(response.status){
			var ddData=response.imgComboList;
			//JSON.stringify
			//alert(JSON.stringify(response.imgComboList));
			
			 $('#partySymbolIdId').ddslick({
				    ...
				    onSelected: function(selectedData){
				        ...
				        partySymbolIdOnImageSelection=selectedData.selectedData.value;				        
				    }   
				});
