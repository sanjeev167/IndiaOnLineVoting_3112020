...
function reloadGrid() {
	...
	window.location.replace("/pub/ec/assemblies/");
...
function searchAndReloadGrid(stateNameId,loksabhaNameId,assemblyName) {
	clearCheckBoxSelection();
	var table = $('#assemblyId').DataTable();
	table.ajax.url(
	...
	$(".chkIndvRow").each(function() {
		if ($(this).is(':checked')) {
			...
			checkedRowNames.push(selectedRows[0].stateName +" => "+
					             ...
					             +selectedRows[0].assemblyName + "</br>");
...
var t;
function loadGrid(stateNameId,loksabhaNameId,assemblyNameId) {
	t = $('#assemblyId').DataTable(
			{
				"responsive": true,
				...
				"ajax" : {
					...
					"url" : "paginated?stateNameId=" + stateNameId + "&loksabhaNameId=" + loksabhaNameId+"&assemblyNameId="+assemblyNameId,
...
var baseUrl = '/pvt/master/assembly';
...
function cleanAllMsg() {
	...
	$('#assemblyNoId_err').html("");
}

function hideAllRequired() {	
	$("#searchMsgId").hide();
	$('.commonButtonActionClass').hide();
	$('#assemblyLabledId').hide();
	$('#assemblyNoLabledId').hide();
}

function cleanAllHiddenInput() {
	$('#assemblyRecordId').val("");
	$('#recordIdForDelete').val("");
	$('#assemblyNoLabledId').show();
}
...
function applyReadOnlyProp() {	
	...
	$('#assemblyNameId').prop("readonly", true);
	$('#assemblyNoId').prop("readonly", true);	
}
...
function removeReadOnlyProp() {	
	...
	$('#assemblyNameId').prop("readonly", false);
	$('#assemblyNoId').prop("readonly", false);	
}
...
function loadRecordInForm(response) {
	$('#assemblyRecordId').val(response.id);	
	...
	$('#assemblyNameId').val(response.assemblyName);
	$('#assemblyNoId').val(response.assemblyNo);
}
...
function showBusinessEerror(fieldErrMsgMap) {	
	...
	$('#assemblyNameId_err').html(fieldErrMsgMap.assemblyName);
	$('#assemblyNoId_err').html(fieldErrMsgMap.assemblyNo);
}
...
function saveAndUpdateRecord() { 
	...
	var id = $('#assemblyRecordId').val();	
	var stateNameId = $('#stateNameId').val();	
	var loksabhaNameId = $('#loksabhaNameId').val();
	var assemblyName = $('#assemblyNameId').val();
	var assemblyNo = $('#assemblyNoId').val();	
